...
export default function EditMenuPage() {
  ...
  useEffect(() => {
    ...
    if (foundMenu) {
      setOriginalMenu(foundMenu)
      // フォームにデータを設定
      setTitle(foundMenu.title)
      setDescription(foundMenu.description || '')
      setSport(foundMenu.sport)
      setClassType(foundMenu.classType)
      setIsTemplate(foundMenu.isTemplate)
      setNotes(foundMenu.notes || '')
      setWarmup(foundMenu.warmup || [])
      setActivities(foundMenu.activities || [])
      setCooldown(foundMenu.cooldown || [])
      setEnglishPhrases(foundMenu.englishPhrases || [])
    }
  }, [params.id])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('更新されたメニュー:', updatedMenu)
    router.push(`/lessons/menus/${params.id}`)
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" asChild>
            <Link href={`/lessons/menus/${params.id}`}>
              <ArrowLeft className="h-4 w-4 mr-2" />
              戻る
            </Link>
          </Button>
          <div>
            <h1 className="text-3xl font-bold tracking-tight">レッスンメニューを編集</h1>
            <p className="text-muted-foreground mt-1">{originalMenu.title}</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" asChild>
            <Link href={`/lessons/menus/${params.id}`}>キャンセル</Link>
          </Button>
          <Button type="submit">
