...
// === 音声システム完全無効化（エラー対策） ===
logger.log('🔇 Initializing MovWISE in silent mode for stability')
...
try {
  ...
  logger.warn('pinia-plugin-persistedstate not installed, using memory storage only')
}

// IPA Audio Serviceは無効化（エラー回避）
logger.log('🔇 IPA Audio Service disabled for stability')
...
try {
  ...
  logger.log('🔇 AudioManager loaded in silent mode')
} catch (error) {
  logger.warn('AudioManager not found, creating dummy')
...
try {
  ...
  logger.warn('main.css not found, using default styles')
}

// 宇宙テーマCSS
try {
  await import('./assets/css/cosmic-theme.css')
  logger.log('✨ Cosmic theme CSS loaded successfully')
} catch (error) {
  logger.warn('cosmic-theme.css not found, using default theme')
}
...
try {
  ...
  logger.warn('Modal components not found, using fallbacks')
...
const initializeAudioManager = async () => {
  try {
    logger.log('🔇 Initializing AudioManager in silent mode...')
    ...
    const dummyAudioManager = {
      play: (sound) => {
        logger.log(`🔇 [Silent] ${sound}`)
        return Promise.resolve()
      },
      playCorrect: () => {
        logger.log('🔇 [Silent] Correct sound')
        return Promise.resolve()
      },
      playIncorrect: () => {
      ...
      setVolume: (volume) => {
        logger.log(`🔇 [Silent] Volume: ${volume}`)
      },
      setEnabled: (enabled) => {
        logger.log(`🔇 [Silent] Enabled: ${enabled}`)
      }
    }

    audioStore.setAudioManager(dummyAudioManager)
    logger.log('✅ Dummy AudioManager initialized successfully')

  } catch (error) {
    logger.warn('⚠️ AudioManager initialization failed:', error)
  }
}

// ユーザージェスチャー処理は無効化（エラー回避）
const enableAudioOnUserGesture = () => {
  logger.log('🔇 User gesture audio enablement disabled')
}
...
app.config.errorHandler = (err, instance, info) => {
  ...
  if (err.message && (
    ...
    logger.log('🔇 Audio error ignored:', err.message)
    return // エラーを無視
  }
...
if (import.meta.env.DEV) {
  nextTick(() => {
    setTimeout(async () => {
      try {
        ...
        logger.log('✅ Test data initialized successfully')
      } catch (error) {
        logger.warn('⚠️ Could not initialize test data:', error)
      }
    }, 500)
  })
  ...
  setTimeout(() => {
    window.testAudio = {
      playCorrect: () => logger.log('🔇 [Debug] Correct sound'),
      playIncorrect: () => logger.log('🔇 [Debug] Incorrect sound'),
      playClick: () => logger.log('🔇 [Debug] Click sound'),
      speak: (text) => logger.log(`🔇 [Debug] TTS: ${text}`)
    }
