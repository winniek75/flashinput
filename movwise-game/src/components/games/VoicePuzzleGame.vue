<template>
  <div class="min-h-screen galaxy-background relative overflow-hidden">
    ...
    <div class="relative z-10 flex-1 flex flex-col items-center justify-center px-4">
      ...
      <!-- Puzzle Game - Compact Layout -->
      <div v-else-if="gameState === 'playing'" class="w-full max-w-7xl">
        <div class="grid grid-cols-1 xl:grid-cols-5 gap-4">
          <!-- Left Control Panel -->
          <div class="xl:col-span-1 space-y-3">
            <!-- Current Target -->
            <div class="galaxy-card rounded-xl p-3">
              <h3 class="text-sm font-bold text-galaxy-moon-silver mb-2">目標ピース</h3>
              
              <div v-if="currentTargetPiece" class="text-center">
                <div class="text-3xl mb-2">{{ currentTargetPiece.emoji }}</div>
                <div class="text-base font-bold text-yellow-400 mb-1">
                ...
                <button
                  ...
                  class="galaxy-button galaxy-button-secondary px-2 py-1 rounded-lg text-xs font-bold mb-2 w-full"
                >
                  <Volume2 class="w-3 h-3 inline mr-1" />
                  音を聞く
                </button>
                ...
                <button
                  ...
                  :class="[
                    'px-3 py-2 rounded-lg text-xs font-bold text-white transition-all w-full mb-2',
                    listening 
                      ? 'galaxy-button-listening animate-pulse' 
                      : 'galaxy-button galaxy-button-primary hover:shadow-lg'
                  ]"
                >
                  <Mic class="w-3 h-3 inline mr-1" />
                  {{ listening ? '聞いています...' : '発音する' }}
                </button>

                <div v-if="recognizedText" class="mb-2">
                  <div class="text-xs text-galaxy-moon-silver">認識:</div>
                  <div class="text-sm font-bold text-yellow-400">{{ recognizedText }}</div>
                </div>

                <div v-if="pronunciationScore > 0">
                  <div class="text-xs text-galaxy-moon-silver mb-1">精度:</div>
                  ...
                  <div class="text-xs font-bold text-white">{{ pronunciationScore }}%</div>
                </div>
              </div>

              <div v-else class="text-center text-galaxy-moon-silver text-xs">
                パズルピースを探しています...
              </div>
            </div>

            <!-- Gaming Stats Compact -->
            <div class="galaxy-card rounded-xl p-3">
              <h3 class="text-sm font-bold text-galaxy-moon-silver mb-2">ゲーミング</h3>
              
              <!-- Energy Bar -->
              <div class="mb-2">
                <div class="flex justify-between text-xs text-galaxy-moon-silver mb-1">
                  <span>エナジー</span>
              ...
              <div class="grid grid-cols-2 gap-2 mb-2">
                <div class="bg-purple-900 rounded-lg p-2 text-center">
                  <div class="text-xs text-purple-300">コンボ</div>
                  <div class="text-sm font-bold text-purple-100">{{ combo }}</div>
                </div>
                <div class="bg-yellow-900 rounded-lg p-2 text-center">
                  <div class="text-xs text-yellow-300">倍率</div>
                  <div class="text-sm font-bold text-yellow-100">×{{ multiplier.toFixed(1) }}</div>
                </div>
              </div>
            </div>
            ...
            <!-- Power-ups Compact -->
            <div class="galaxy-card rounded-xl p-3">
              <h3 class="text-sm font-bold text-galaxy-moon-silver mb-2">パワーアップ</h3>
              
              <div class="grid grid-cols-2 gap-1 mb-2">
                <button
                  @click="activatePowerUp('timeFreeze')"
                  :disabled="powerUps.timeFreeze <= 0 || isTimeFrozen"
                  class="px-2 py-1 rounded text-xs font-bold transition-all disabled:opacity-50"
                  :class="isTimeFrozen ? 'bg-blue-600 text-white' : 'galaxy-button galaxy-button-secondary'"
                >
                  🧊{{ powerUps.timeFreeze }}
                </button>
                ...
                <button
                  ...
                  class="px-2 py-1 rounded text-xs font-bold transition-all disabled:opacity-50"
                  :class="isDoublePoints ? 'bg-green-600 text-white' : 'galaxy-button galaxy-button-secondary'"
                >
                ...
                <button
                  ...
                  class="px-2 py-1 rounded text-xs font-bold transition-all disabled:opacity-50"
                  :class="isPerfectBoost ? 'bg-pink-600 text-white' : 'galaxy-button galaxy-button-secondary'"
                >
                  🎯{{ powerUps.perfectBoost }}
                </button>
                
                <button
                  @click="useHint"
                  :disabled="hints <= 0 && powerUps.autoHint <= 0"
                  class="px-2 py-1 rounded text-xs font-bold transition-all disabled:opacity-50 galaxy-button galaxy-button-secondary"
                >
                  💡{{ hints + powerUps.autoHint }}
                </button>
              </div>
              
              <div v-if="currentHint" class="text-xs text-yellow-400 bg-yellow-900 p-2 rounded">
                {{ currentHint }}
              </div>
            </div>
          </div>
          ...
          <div class="xl:col-span-3">
            <div class="galaxy-card rounded-2xl p-4">
              <h3 class="text-lg font-bold text-galaxy-moon-silver mb-3 text-center">
              ...
              <div 
                ...
                :style="{
                  width: `${selectedDifficulty.gridSize * 70}px`,
                  height: `${selectedDifficulty.gridSize * 70}px`
                }"
              >
                <!-- Background Grid -->
                ...
                <div 
                  ...
                  :style="{
                    left: `${piece.x * 70 + piece.x}px`,
                    top: `${piece.y * 70 + piece.y}px`,
                    width: '70px',
                    height: '70px',
                ...
                >
                  <div class="piece-overlay">
                    <div class="text-xl">{{ piece.emoji }}</div>
                  </div>
                </div>
              ...
              <!-- Progress Bar -->
              <div class="mt-4">
                <div class="text-sm text-galaxy-moon-silver mb-2 text-center">完成度: {{ Math.round(puzzleProgress) }}%</div>
                <div class="w-full bg-gray-700 rounded-full h-3">
                  <div 
                    class="h-3 rounded-full transition-all duration-1000 bg-gradient-to-r from-blue-500 to-purple-500"
          ...
          <div class="xl:col-span-1">
            <div class="galaxy-card rounded-xl p-3">
              <h3 class="text-sm font-bold text-galaxy-moon-silver mb-2">ピース一覧</h3>
              ...
              <div class="space-y-1 max-h-96 overflow-y-auto">
                <div 
                  ...
                  :key="piece.id"
                  :class="[
                    'piece-item flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-all text-xs',
                    piece.activated 
                      ? 'bg-green-900 border border-green-400 piece-glow' 
                ...
                >
                  <div class="text-lg">{{ piece.emoji }}</div>
                  <div class="flex-1">
                    <div class="text-xs font-bold text-white">{{ piece.word }}</div>
                    <div class="text-xs text-gray-400">{{ piece.type }}</div>
                  </div>
                  <div v-if="piece.placed" class="text-green-400 text-sm">✅</div>
                  <div v-else-if="piece.activated" class="text-yellow-400 animate-pulse text-sm">✨</div>
                </div>
              </div>
            </div>
          </div>
